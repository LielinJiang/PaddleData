...
template <typename T>
void copy_kernelLauncher(const T* in,
                               T* out,
                               const int numel,
                               cudaStream_t stream);
...
class ImageDecoder {
  public:
    ...
    void Run(const uint8_t* bit_stream, size_t bit_len, paddle::Tensor* out, 
...
class ImageDecoderThreadPoolManager {
 ...
 public:
  ...
  ImageDecoderThreadPoolManager() {
    std::cout << "ImageDecoderThreadPoolManager init"<< std::endl;
    // VLOG(1) << "ImageDecoderThreadPoolManager init";
  }

  ~ImageDecoderThreadPoolManager() {
    std::cout << "~DecoderThreadPoolManager" << std::endl;
