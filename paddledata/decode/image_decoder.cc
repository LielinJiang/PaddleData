...
void ImageDecoder::CPUDecodeRandomCrop(
                        ...
                        paddle::Tensor* out, paddle::PlaceType place) {
...
// #ifdef PADDLE_WITH_OPENCV
  ...
  std::cout << "cpu decode 00" << std::endl;
  ...
  std::cout << "cpu decode 02" << std::endl;

  std::cout << "cpu decode 02.5 "<<(int)cpu_mat.data[0] << " "<< (int)cpu_mat.data[1] << " " << (int)cpu_mat.data[numel - 1] << std::endl;
  out->reshape(out_shape);
  auto* out_data = out->mutable_data<uint8_t>(place);
  cudaMemcpyAsync(out_data, cpu_mat.data, numel * sizeof(uint8_t), cudaMemcpyHostToDevice, out->stream());

  std::cout << "cpu decode 04" << std::endl;

}

nvjpegStatus_t ImageDecoder::ParseDecodeParams(
    const uint8_t* bit_stream, size_t bit_len, paddle::Tensor* out,
